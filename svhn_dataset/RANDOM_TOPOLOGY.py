...
def fix_topology(dict_model):
    ...
    for key,value in dict_model.items():
        ...
        if len(value) < 2:
            temp_dict[KEY_SECOND_LAYER] = EMPTY_LAYER
            temp_dict[KEY_THIRD_LAYER] = EMPTY_LAYER
            temp_dict[KEY_FORTH_LAYER] = EMPTY_LAYER
...
def add_layer(number_model):
    ...
    while num_model < number_model:
        ...
        for num_layer in range(MAX_NUM_LAYER):
            ...
            if num_layer == 0:
                if element == SOFTMAX_LAYER or element == 'm_1' or element == 'm_2' or element == 'm_3' :
                    continue
            if num_layer == 1:
                if element == SOFTMAX_LAYER:
                    continue
            temp_list.append(element)

            if element == SOFTMAX_LAYER:
                break
        if len(temp_list) < MAX_NUM_LAYER and temp_list[-1] != SOFTMAX_LAYER:
...
def fix_dict(dict_data):
    ...
    for array in data:
        ...
        for single_element in array:
            if single_element != EMPTY_LAYER:
                count += 1
        if count < MAX_NUM_LAYER and array[count-1] != SOFTMAX_LAYER:
            array[count] = SOFTMAX_LAYER
            temp_array[count+1] = SOFTMAX_LAYER
